...
import WalletGame from './walletGame';
...
class WalletPage extends Component {
  ...
  render(){
    ...
    <ul className='pendingWagers'>
      { this.state.pendingWagers.map(w => <li> You have ${w.wager.amount} wagered on </li>)}
    </ul>)
    let completedWagers = (
    <ul className='completedWagers'>
      { this.state.completedWagers.map(w => <li> You have ${w.wager.amount} wagered on </li>)}
    </ul>)
