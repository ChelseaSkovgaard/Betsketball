...
class GamePage extends Component {
  ...
  setWager(location){
    ...
    let wager= {key:key, amount:userInput, finished:false, outcome:outcome }
  ...
  render(){
    // this belongs in the map < Game game={g}, setWager=setWager />
    let userBudget = this.props.userBudget;
    let user = this.props.user;
    let gamesList = (<ul className='games-list'>{this.state.todaysGames.map(g =>
      <Game
    ...
    return (
      <div className="game-page">
        <h2 className='game-schedule-header'>Todays Schedule</h2>
        {gamesList}
      </div>
    );
  }
