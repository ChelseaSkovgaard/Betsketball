...
class Game extends Component {
  ...
  render(){
    ...
    return(
      ...
      <article className='date-time'> {this.props.game.date} {this.props.game.time} </article>
      <article>
        <button
        ...
        <span className='bet-input'>
          ...
          />
          <button aria-label="submit-wager-button"
            onClick={this.submitWager.bind(this)}> Bet!
          </button>
          <span className="bet-submit-message">{this.state.message}</span>
        </span>
       </article>
      <article>{this.props.game.location}</article>
      </section>
    )
  }
}
